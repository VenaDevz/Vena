...
/**
 * @title VenaMiningV3 (Robinhood Chain)
 * @notice Buyback-fed staking with Stratum (per-NFT stake time) + optional Loadout multiplier.
 *         Loadout shop/level flags can stay off until UI launch — staking works without bonuses.
 */
contract VenaMiningV3 is IERC721Receiver, Ownable, ReentrancyGuard {
    ...
    function syncPower() external nonReentrant {
        require(started, "Mining not started");
        UserInfo storage user = _users[msg.sender];
